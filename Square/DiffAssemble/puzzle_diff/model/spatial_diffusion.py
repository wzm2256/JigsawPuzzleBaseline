...
class GNN_Diffusion(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if not self.all_equivariant:
            if batch_idx == 0 and self.local_rank == 0:
                ...
                save_path = Path(f"results/{self.logger.experiment}/train")
                for i in range(
            ...
            for i in range(batch.batch.max() + 1):
                ...
                if not self.all_equivariant:
                    if (
                        ...
                        save_path = Path(f"results/{self.logger.experiment}/val")
    ...
    def test_step(self, batch, batch_idx):
        with torch.no_grad():
            ...
            for i in range(batch.batch.max() + 1):
                ...
                y = torch.linspace(-1, 1, n_patches[0], device=self.device)
                x = torch.linspace(-1, 1, n_patches[1], device=self.device)
                xy_raw = torch.stack(torch.meshgrid(x, y, indexing="xy"), -1)

                piece_accuracy_4 = 0.0
                for r_i in range(4):

                    xy = torch.rot90(xy_raw, r_i, (0, 1))
                    real_grid = einops.rearrange(xy, "x y c-> (x y) c")
                    ...
                    if self.rotation:
                        rotation_matrix = torch.linalg.matrix_power(torch.tensor([[0., -1.], [1., 0.]], device=img.device), r_i)
                        pred_rot = img[idx, 2:]
                        # gt_rot = batch.x[idx, 2:]

                        gt_rot = einops.einsum(rotation_matrix, batch.x[idx, 2:], 'i j, b j -> b i')
                        ...
                        correct = correct and rot_correct.all()
                        piece_accuracy = rot_correct * piece_accuracy
                        ratio = piece_accuracy.sum() / piece_accuracy.shape[0]
                        # print(ratio)
                        if ratio > piece_accuracy_4:
                            piece_accuracy_4 = ratio
                ...
                if self.save_eval_images:
                    for id_t, t_img in enumerate(imgs):
                        ...
                        save_path = Path(f"results/{self.logger.experiment}/test")
                    ...
                    img_plot.save(os.path.join(save_path, f"{i_name}_{id_t:03d}.png") )
                    ...
                    save_path = f"results/{self.logger.experiment}/test"
                    save_path = Path(save_path)
                    save_path.mkdir(parents=True, exist_ok=True)
                    fig.savefig(
    ...
    def save_image_rotated(
        ...
        gt_img_correct.save(f"{file_name}/{self.current_epoch}-{ind_name}_gt.png")
        gt_img.save(f"{file_name}/{self.current_epoch}-{ind_name}_calibrate.png")
        pred_img.save(f"{file_name}/{self.current_epoch}-{ind_name}_raw.png")
