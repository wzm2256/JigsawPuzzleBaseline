...
function [res_all_sol, err_all, real_goal_all, res_all, rotsNum_first, rotsNum, large_Image, ratio] = solve_type_2_img(img_path, patch_size, cols, rows, numb_alg_iter)
...
rotation_record = randscram - 1;


% rotPatches_tmp = raw_ater_permutation;
% for jj = 1:1:numel(ap)
%     rotPatches_tmp{jj} = imrotate(raw_ater_permutation{jj},90 * rotation_record(jj));
%     disp(max(max(max(abs(rotPatches_tmp{jj} - ap{jj})))))
% end


% large_Image = printLargeImage(ap, cols, rows, real_goal(:));
...
%Calculate rotations
    eig_num = 5;
    [ rotPatches, rotsNum, rotations, A_for_VDD, rotate_now ] = calculate_rotations( nn4Mat + diag_nb/2, rot_mat, rotPatches, eig_num);
    
    rotation_record = mod(rotation_record + transpose(rotate_now), 4);
    % rotPatches_tmp = raw_ater_permutation;
    % for jj = 1:1:numel(ap)
    %     rotPatches_tmp{jj} = imrotate(raw_ater_permutation{jj},90 * rotation_record(jj));
    %     disp(max(max(max(abs(rotPatches_tmp{jj} - rotPatches{jj})))));
    % end
...
% using Yu et al algorithm to find the locations
    ...
    rotation_record_tmp = rotation_record;
    for i = 1:(cols*rows)
        rot_patches_temp{i} = rot90(rot_patches_temp{i});
        rotation_record_tmp(i) = mod(rotation_record(i) + 1, 4);
    end
    ...
    if(err_all(1) > err_all(2))
        if(size(real_goal_1, 1) * size(real_goal_1, 2) >= size(real_goal, 1) * size(real_goal, 2))
            ...
            rotation_record = rotation_record_tmp;
...
%         imshow(large_Image);
        ...
        try
            predict_piece0 = ppp(real_goal);
            true_piece_1d = 1:1:numel(real_goal(:));
